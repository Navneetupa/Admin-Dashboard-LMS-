...
export default function EnhancedSidebar({ onLinkClick }) {
  ...
  return (
    <div
      className="w-64 h-full flex flex-col justify-between rounded-2xl shadow-xl p-5 transition-all duration-300"
      ...
      <div className="mt-8">
        <div className="flex items-center gap-3 mb-4 p-2 rounded-lg hover:bg-[var(--card-bg)] transition-all duration-200">
          <img
            ...
            className="rounded-full w-12 h-12 object-cover ring-2 ring-[var(--accent-color)]/30"
          />
          <div>
            <p className="text-sm font-semibold">{`${user.firstName} ${user.lastName}`}</p>
            <span className="text-xs bg-[var(--accent-color)]/10 text-[var(--accent-color)] px-2 py-1 rounded-full">
              {user.role}
            </span>
          </div>
        </div>
        <div className="space-y-1.5">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              isActive
                ? `${linkClass} ${activeClass} text-red-500`
